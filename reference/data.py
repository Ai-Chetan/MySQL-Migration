...
def show_schema(event=None):
    ...
    new_schema_defs = get_schema_file_definition(selected_table_db) 
    ...
    if new_schema_file_def: 
        schema_frame_new.config(text=f"New Schema (from File: {target_schema_name_for_display})") 
...
def get_merge_copy_details(merge_key, current_mappings, parsed_schema):
    ...
    join_conditions = map_info.get("join_conditions", "") # This is the user-provided SQL snippet for JOINs
    ...
    for new_col_name, new_col_definition in new_schema_dict.items():
        ...
        try:
            ...
            fatal_error = f"Invalid source specifier '{source_specifier}' for target column '{new_col_name}'. Expected 'table.column'."
            break
    ...
    from_clause = f"`{source_tables[0]}`" # Start with the first source table
    # Append the user-provided JOIN conditions directly
...
def map_columns():
    ...
    def unmap_selected_col():
        ...
        selected_new_disp = new_col_var.get() 
